...
public class ResourceExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
	    ...
	    List<FieldMessage> problemFields = bindingResult.getFieldErrors().stream()
	    		.map(fieldError -> {
	    			String message = messageSource.getMessage(fieldError, LocaleContextHolder.getLocale());
	    				return FieldMessage.builder()
	...
	public ResponseEntity<?> handlerObjectNotFund(ObjectNotFoundException ex, WebRequest request) {
		...
		Problem problem = createProblemBuilder(status, ProblemType.RECURSO_NAO_ENCONTRADO, ex.getMessage()).userMessage(ex.getMessage()).build();
	...
	public ResponseEntity<?> hendlerDataViolation(ViolationException ex, WebRequest request) {
		...
		Problem problem = createProblemBuilder(status, ProblemType.ERRO_NEGOCIO, ex.getMessage()).userMessage(ex.getMessage()).build();
	...
	public ResponseEntity<?> hendlerDataIntegrityViolation(DataIntegrityException ex, WebRequest request) {
		...
		Problem problem = createProblemBuilder(status, ProblemType.RECURSO_EM_USO, ex.getMessage()).userMessage(ex.getMessage()).build();
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception e, Object body, HttpHeaders headers,
		...
		if (body == null) {
			body = Problem.builder().status(status.value()).title(status.getReasonPhrase()).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).build();
		} else if (body instanceof String) {
			body = Problem.builder().status(status.value()).title((String) body).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).build();
		}

		return super.handleExceptionInternal(e, body, headers, status, request);
	}
	
	//Geração do builder de problem
	private Problem.ProblemBuilder createProblemBuilder(HttpStatus status, ProblemType problemType, String detail) {

		return Problem.builder().timestamp(LocalDateTime.now()).status(status.value()).type(problemType.getUri()).title(problemType.getTitle())
				.detail(detail);
	}
